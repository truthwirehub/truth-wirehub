  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        if (!url || !key) {
          console.log("Env variables missing locally.")
          return
        }
        const res = await fetch(
          `${url}/rest/v1/crypto_data?select=ai_analysis,date_recorded&order=id.desc&limit=3`,
  ...
  useEffect(() => {
    ...
    const initAnimations = async () => {
      ...
      document.addEventListener('mousemove', e => {
        gsap.to(cur, { x: e.clientX, y: e.clientY, duration: 0.08, ease: 'power2.out' })
    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #cursor { position: fixed; width: 12px; height: 12px; background: var(--g); border-radius: 50%; pointer-events: none; z-index: 9999; transform: translate(-50%,-50%); mix-blend-mode: difference; transition: width .2s, height .2s; }
        ...
        .welcome-sub { margin-top: 40px; font-size: clamp(1rem, 2vw, 1.3rem); color: rgba(255,255,255,0.28); letter-spacing: 1px; line-height: 1.7; max-width: 600px; margin-left: auto; margin-right: auto; font-weight: 300; min-height: 60px; }
        ...
        .report-card { background: rgba(0,255,180,0.03); padding: 30px; border-radius: 8px; border: 1px solid rgba(0,255,180,0.2); margin-bottom: 40px; backdrop-filter: blur(10px); position: relative; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.5), inset 0 0 20px rgba(0,255,180,0.05); transform: perspective(1000px) rotateX(2deg); transition: all 0.4s ease; cursor: pointer; }
        .report-card:hover { transform: perspective(1000px) rotateX(0deg) translateY(-10px) scale(1.02); border-color: rgba(0,255,180,0.8); box-shadow: 0 30px 60px rgba(0,255,180,0.1); }
        .scanline { position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, transparent, #00ffb4, transparent); animation: scan 3s linear infinite; }
        .access-btn { display: inline-block; margin-top: 10px; color: #00ffb4; font-size: 0.7rem; text-decoration: none; border: 1px solid #00ffb4; padding: 10px 25px; letter-spacing: 2px; text-transform: uppercase; background: transparent; transition: all 0.3s ease; }
        .access-btn:hover { background: #00ffb4; color: #000; }
        .archive-btn { padding: 18px 45px; border: 1px solid var(--g); color: #00ffb4; text-decoration: none; font-size: 0.75rem; letter-spacing: 4px; text-transform: uppercase; border-radius: 2px; background: rgba(0,255,180,0.03); font-weight: bold; transition: all 0.3s ease; }
        .archive-btn:hover { background: rgba(0,255,180,0.1); box-shadow: 0 0 30px rgba(0,255,180,0.2); }
      `}} />
      ...
      <section className="scene" id="scene-welcome">
        ...
        <p className="welcome-sub" id="welcome-sub"></p>
      ...
      <section className="scene" id="scene-reports" style={{ alignItems: 'flex-start', padding: '100px 10vw' }}>
        ...
        <div style={{ width: '100%', maxWidth: '900px' }}>
          {reports.length === 0 ? (
            <p style={{ color: 'rgba(255,255,255,0.4)', letterSpacing: '2px', fontSize: '0.85rem' }}>
            ...
            reports.slice(0, 3).map((rep, idx) => (
              <div key={idx} className="report-card">
                ...
                <h3 style={{ fontSize: '1.6rem', color: '#fff', margin: '15px 0', fontWeight: 900, letterSpacing: '-1px' }}>
                  ENCRYPTED_INTEL_STREAM #{idx + 1}
                </h3>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem', letterSpacing: '1px', marginBottom: '15px' }}>
                  {rep.ai_analysis ? rep.ai_analysis.substring(0, 120) + '...' : 'Classified intelligence briefing. Access required.'}
                </p>
                <Link href="/sequence" className="access-btn">
                  ACCESS_SEQUENCE
                </Link>
              </div>
            ))
